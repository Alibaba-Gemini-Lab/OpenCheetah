...
def load_net(data_path):
    if not os.path.isfile(data_path):
        parser.error(
            "Network %s does not exist. (Did you forget to download it?)" % data_path
        )
...
def net_preloaded(
    ...
    x = _act_layer(net, layer_name + "_actv", x)
    x = _pool_layer(
        net, "pool1_pool", x, pooling, size=(3, 3), stride=(2, 2), padding="VALID"
    )

    # fire2 + fire3 clusters
    x = fire_cluster(
        net, x, preloaded, cluster_name="fire2", runPrediction=runPrediction
    )
    fire2_bypass = x
    x = fire_cluster(
        net, x, preloaded, cluster_name="fire3", runPrediction=runPrediction
    )
    x = _pool_layer(
        net, "pool3_pool", x, pooling, size=(3, 3), stride=(2, 2), padding="VALID"
    )

    # fire4 + fire5 clusters
    x = fire_cluster(
        net, x, preloaded, cluster_name="fire4", runPrediction=runPrediction
    )
    fire4_bypass = x
    x = fire_cluster(
        net, x, preloaded, cluster_name="fire5", runPrediction=runPrediction
    )
    x = _pool_layer(
        net, "pool5_pool", x, pooling, size=(3, 3), stride=(2, 2), padding="VALID"
    )

    # remainder (no pooling)
    x = fire_cluster(
        net, x, preloaded, cluster_name="fire6", runPrediction=runPrediction
    )
    fire6_bypass = x
    x = fire_cluster(
        net, x, preloaded, cluster_name="fire7", runPrediction=runPrediction
    )
    x = fire_cluster(
        net, x, preloaded, cluster_name="fire8", runPrediction=runPrediction
    )
    x = fire_cluster(
        net, x, preloaded, cluster_name="fire9", runPrediction=runPrediction
    )
    ...
    if needs_classifier == True:
        ...
        weights, biases = get_weights_biases(preloaded, layer_name)
        x = _conv_layer(
            net, layer_name + "_conv", x, weights, biases, runPrediction=runPrediction
        )
        x = _act_layer(net, layer_name + "_actv", x)

        # Global Average Pooling
        x = tf.nn.avg_pool(
            x, ksize=(1, 13, 13, 1), strides=(1, 1, 1, 1), padding="VALID"
        )
...
def _conv_layer(
    net, name, input, weights, bias, padding="SAME", stride=(1, 1), runPrediction=True
):
    global all_weights
    if runPrediction:
        conv = tf.nn.conv2d(
        ...
        conv = tf.nn.conv2d(
            input,
            tf.Variable(tf.constant(0.1, shape=weights.shape)),
            strides=(1, stride[0], stride[1], 1),
            padding=padding,
        )
...
def main():
    ...
    image_names = glob.glob('imagenet-sample-images/*.JPEG')
    g = tf.Graph()
    # 1st pass - simple classification
    with g.as_default(), tf.Session(config=config) as sess:
        # Building network
        img_content_shape = [1, 227, 227, 3]
        image = tf.placeholder(
            dtype=get_dtype_tf(), shape=img_content_shape, name="image_placeholder"
        )
        keep_prob = 0.0
        saveTFMetadata = False
        sqznet = net_preloaded(
            data, image, "max", True, keep_prob, runPrediction=not (saveTFMetadata)
        )
        final_class = tf.argmax(sqznet["classifier_pool"], 3)
        sess.run(tf.global_variables_initializer())

        for image_name in image_names:
            img_content, orig_shape = imread_resize(image_name)
            ...
            _fname = image_name.split('/')[1].split('.')[0]

            DumpTFMtData.dumpImageDataInt(
                    imageData,
                    "preprocessed/{}_scale{}_class({})_pred{}.inp".format(_fname, options.scalingFac, classes[sqz_class], sqz_class),
